...
class Data:
    ...
    def __init__(self, X=None, Y=None, file=None, frame=None, X_key=None, Y_key=None, *args, **kwargs):
        ...
        Possibility 2: Via a Dataframe and column names for X, Y, (Sigma)
        """
                ...
                if file[-4:] == ".csv":
                    ...
                    if X_key is not None:
                        if X_key == "__index__" and "__index__" not in f.keys():
                            self.X=torch.tensor(f.index)
                        else:
                            self.X = torch.tensor(f[X_key])
                    if Y_key is not None:
                        self.Y = torch.tensor(f[Y_key])
                elif file[-4:] == ".mat":
                    f = io.loadmat(file)
                    if X_key is not None:
                        self.X = torch.tensor(f[X_key].reshape((len(f[X_key]),)))
                    if Y_key is not None:
                        self.Y = torch.tensor(f[Y_key].reshape((len(f[Y_key]),)))
                else:
    ...
    def time_to_X(self, file=None, frame=None, label_year=None, label_month=None, label_day=None, label_hour=None, label_minute=None, label_second=None):
        if frame == None and isinstance(file, str):
            ...
            self.X = torch.tensor(time_ticks, dtype=torch.float)
