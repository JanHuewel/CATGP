...
# TODO: make sure that train and test data are normalized equally!!
class ExactGPModel(gpt.models.ExactGP):
    ...
    def get_current_loss(self):
        # NOTE: gpytorch divides the mll by number of training points
        self.train()
    ...
    def train_model(self, model_evaluation_function = None):
        self.train()
        self.likelihood.train()
        optimizer = torch.optim.SGD([{"params": self.parameters()}], lr=options["training"]["learning_rate"])
        mll = gpt.mlls.ExactMarginalLogLikelihood(self.likelihood, self)

        for i in range(options["training"]["max_iter"]):
            optimizer.zero_grad()
            if model_evaluation_function:
                loss = model_evaluation_function(self)
            else:
                output = self.__call__(self.train_inputs[0])
                loss = -mll(output, self.train_targets)
            loss.backward()
    ...
    def optimize_hyperparameters(self, model_evaluation_function = None):
        """
        ...
        for iteration in range(options["training"]["restarts"] + 1):
            ...
            try:
                self.train_model(model_evaluation_function)
                if model_evaluation_function:
                    current_loss = model_evaluation_function(self)
                else:
                    current_loss = self.get_current_loss()
            except:
                if options["training"]["print_optimizing_output"]:
                    print("TRAINING: Parameterization failed, skipping restart")
            ...
            # set new random initial values
            self.likelihood.noise_covar.noise = torch.rand(1) * (limits["Noise"][1] - limits["Noise"][0]) + \
                                                limits["Noise"][0]
            ...
            for kernel in get_kernels_in_kernel_expression(self.covar_module):
                try:
                    ...
                    for hyperparameter in hypers:
                        setattr(kernel, hyperparameter,
                                torch.rand(1) * (hypers[hyperparameter][1] - hypers[hyperparameter][0]) +
                                hypers[hyperparameter][0])
                except KeyError:
                    for name, param in kernel.named_parameters():
                        param.data.fill_(torch.rand(1)[0])

            # print output if enabled
            if options["training"]["print_optimizing_output"]:
                print(
                    f"HYPERPARAMETER OPTIMIZATION: Random Restart {iteration}: loss: {current_loss}, optimal loss: {best_loss}")
...
class ExactStreamGPModel(ExactGPModel):
    ...
    def process_data_cpd(self, update : str = "AKS", base_kernels : list = [], iterations : int = 3, max_complexity : int = 5, cpd_threshold : float = 5.0, cpd_tolerance_outer = 0.5):
        from kernelSearch import AKS, CKS, sop_CKS
        ...
        while self.window_start_index + self.window_size < len(self.data):
            ...
            next_prediction = self.predict(next_point.X)
            self.predictions.append(next_prediction)
            #next_likelihood = self.get_ll(X=next_point.X, Y=next_point.Y)

            # CPD
            #change_point_detected = False
            # CUSUM
            #cpd_cusum_score = max(0, cpd_cusum_score + abs(next_prediction-next_point.Y) - cpd_tolerance_outer)
            cpd_cusum_score = max(0, cpd_cusum_score + abs(next_prediction-next_point.Y) - cpd_tolerance_outer*2*torch.sqrt(self.likelihood.noise).item())
            #cpd_cusum_score = max(0, cpd_cusum_score - abs(self.get_ll(X=next_point.X, Y=next_point.Y)) + cpd_tolerance_outer)
            #current_diff = abs(next_prediction - next_point.Y) - 2*torch.sqrt(self.likelihood.noise).item()
            #if current_diff >= 0:
            #    cpd_cusum_score = max(0, cpd_cusum_score + current_diff - cpd_tolerance_outer*2*torch.sqrt(self.likelihood.noise).item())
            #else:
            #    cpd_cusum_score = max(0, cpd_cusum_score - current_diff - cpd_tolerance_inner*2*torch.sqrt(self.likelihood.noise).item())
            if cpd_cusum_score > cpd_threshold:# * 2*torch.sqrt(self.likelihood.noise).item():
            ...
            if change_point_detected:
                if update == "AKS":
                    ...
                    self.covar_module, self.likelihood = sop_CKS(self.window, self.likelihood, base_kernels, iterations)
                elif update == "HPO":
                    self.optimize_hyperparameters()
                else:
...
class StationaryStreamGPModel(ExactStreamGPModel):
    """
    A subclass of window-based GPs that are restricted to equi-distant data and stationary kernels for fast inference
    """
    def __init__(self, data, likelihood, kernel, window_size):
        assert kernel.is_stationary, "kernel must be stationary"
        super(self).__init__(data, likelihood, kernel, window_size)

    def init_fast_inference(self, number_of_predictions):
        K1 = self.covar_module(self.window.X, self.data[self.window_start_index+self.window_size:self.window_start_index+self.window_size+number_of_predictions].X)
        K2 = self.covar_module(self.window.X, self.window.X) + self.likelihood.noise * torch.eye(self.window_size)
        self.fast_pred_base = K2.inv_matmul(torch.eye(self.window_size), K1)
        self.fast_cov = self.covar_module(self.data[self.window_start_index+self.window_size:self.window_start_index+self.window_size+number_of_predictions].X) - self.fast_pred_base @ K1.mT

    def predict(self, x):
        raise NotImplementedError("Efficient prediction needs to be implemented")
